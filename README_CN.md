...
- **号池模式** 现在支持维护多个认证目录，但运行时仍保持稳定的单当前池语义：任意时刻只有一个 `active-path` 真正参与请求路由。
- 切换当前号池时会在当前服务内立即热生效：同步新的 `auth-dir`、恢复该号池自己的轮询策略，并把 watcher 切换到新的目录，无需重启服务。
- 当号池模式开启时，管理端的使用统计和认证文件操作默认只作用于当前号池；如果需要，管理界面仍可切换为查看全部号池。
- `routing.session-affinity` 与 `routing.session-affinity-ttl` 已经接入当前运行时 selector 体系，可通过配置热更新立即生效。
- 管理接口返回的 key 列表已经补齐稳定的 `auth-index` 元数据，方便管理面板把运行中的认证状态精确映射回配置项。
- Codex 图片生成响应已经补齐 OpenAI chat-completions 与 Gemini 兼容翻译，图片结果可以被下游客户端直接消费，而不是只看到原始事件。
- 使用统计持久化已经按号池分文件保存。Windows 打包桌面版默认写入 `D:\CLIProxyAPI\use`，启动时会恢复当前号池最近 7 天的数据。
- Windows 托盘模式会在重启或退出前先保存 usage 数据，支持从托盘打开管理面板，并在托盘启动路径下主动分离控制台，避免桌面版继续依赖可见的任务栏控制台窗口。
...
版本号来源、检查更新逻辑和本地构建流程见 [VERSIONING.md](VERSIONING.md)。

- 当前分支已经进入统一的 `2.x` 版本线。除非发布说明另有声明，后端与管理面板应保持同一个主版本节奏。

- 使用 `scripts/build-local.ps1` 构建的本地桌面版默认关闭远程模型目录刷新，优先使用内置模型目录。
- 现在可以在管理面板中通过 `model-catalog.remote-refresh-enabled` 对应的开关显式启用或关闭远程模型目录刷新。
- 在没有 CLI 强制覆盖时，这个开关会对当前服务立即生效，不需要重启 CPA。
- `--remote-model` 会强制开启当前进程的远程刷新，`--local-model` 会强制关闭当前进程的远程刷新。
- 当进程被 CLI 参数强制覆盖时，管理面板中的修改仍会写入配置文件，但不会改变当前进程的实际运行状态。
