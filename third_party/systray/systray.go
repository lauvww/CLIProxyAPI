...
// SetTrayClickHandler registers a callback that runs when the tray icon itself
// is left-clicked. When unset, the native default behaviour remains unchanged.
func SetTrayClickHandler(handler func()) {
	trayClickMu.Lock()
	trayClickFunc = handler
	trayClickMu.Unlock()
}
...
func systrayTrayClicked() bool {
	trayClickMu.RLock()
	handler := trayClickFunc
	trayClickMu.RUnlock()
	if handler == nil {
		return false
	}
