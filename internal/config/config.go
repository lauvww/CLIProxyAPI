...
type Config struct {
	...
	// AuthPool stores optional auth directory pools and the currently active pool path.
	// The active path is synced to AuthDir at runtime.
	AuthPool AuthPoolConfig `yaml:"auth-pool,omitempty" json:"auth-pool,omitempty"`
...
// AuthPoolConfig stores configurable auth directory pools.
...
// NormalizeAuthPool normalizes auth-pool values and keeps AuthDir in sync with the active path.
func (cfg *Config) NormalizeAuthPool() {
	if cfg == nil {
		return
	}
	...
	for _, item := range cfg.AuthPool.Paths {
		appendPath(item)
	}

	active := normalizeAuthPoolPath(cfg.AuthPool.ActivePath)
	authDir := normalizeAuthPoolPath(cfg.AuthDir)
	if authDir != "" {
		appendPath(authDir)
	}
	...
	if cfg.AuthPool.Enabled {
		...
		if active == "" && len(paths) > 0 {
			active = paths[0]
		}
		if active != "" && !authPoolPathExists(paths, active) {
			appendPath(active)
		}
		if active != "" {
			authDir = active
		...
		if authDir != "" {
			active = authDir
			appendPath(authDir)
		} else if active == "" && len(paths) > 0 {
			active = paths[0]
			authDir = active
		}
	}

	normalizedStrategies := make(map[string]string)
	for rawPath, rawStrategy := range cfg.AuthPool.RoutingStrategyByPath {
		normalizedPath := normalizeAuthPoolPath(rawPath)
		if normalizedPath == "" {
			continue
		}
		normalizedStrategy, ok := normalizeAuthPoolRoutingStrategy(rawStrategy)
		if !ok {
			continue
		}
...
func (cfg *Config) SyncAuthPoolFromAuthDir() {
	...
	cfg.AuthPool.ActivePath = authDir
	if !authPoolPathExists(cfg.AuthPool.Paths, authDir) {
		cfg.AuthPool.Paths = append(cfg.AuthPool.Paths, authDir)
	}
	cfg.NormalizeAuthPool()
}
...
func (cfg *Config) RuntimeAuthPoolPaths() []string {
	...
	if !cfg.AuthPool.Enabled {
		if authDir := normalizeAuthPoolPath(cfg.AuthDir); authDir != "" {
...
func (cfg *Config) CurrentAuthPoolPath() string {
	if cfg == nil {
		return ""
	}
	if cfg.AuthPool.Enabled {
	...
	return normalizeAuthPoolPath(cfg.AuthDir)
}

// SetAuthPoolEnabled toggles auth pool mode and keeps related values synchronized.
func (cfg *Config) SetAuthPoolEnabled(enabled bool) {
	if cfg == nil {
		return
	}
	cfg.AuthPool.Enabled = enabled
	cfg.NormalizeAuthPool()
}

// SetCurrentAuthPoolPath switches the active auth pool path and updates AuthDir when required.
func (cfg *Config) SetCurrentAuthPoolPath(path string) {
	if cfg == nil {
		return
	}
	normalized := normalizeAuthPoolPath(path)
	...
	cfg.AuthPool.ActivePath = normalized
	if !authPoolPathExists(cfg.AuthPool.Paths, normalized) {
		cfg.AuthPool.Paths = append(cfg.AuthPool.Paths, normalized)
	}
	cfg.AuthDir = normalized
	cfg.NormalizeAuthPool()
}

// SetCurrentAuthPoolRoutingStrategy stores a strategy for the active auth pool.
func (cfg *Config) SetCurrentAuthPoolRoutingStrategy(strategy string) bool {
	if cfg == nil {
		return false
	}
	normalizedStrategy, ok := normalizeAuthPoolRoutingStrategy(strategy)
	if !ok {
		return false
	}
	active := normalizeAuthPoolPath(cfg.CurrentAuthPoolPath())
	if active == "" {
		return false
	}
	if cfg.AuthPool.RoutingStrategyByPath == nil {
		cfg.AuthPool.RoutingStrategyByPath = make(map[string]string)
	}
	cfg.AuthPool.RoutingStrategyByPath[active] = normalizedStrategy
	cfg.Routing.Strategy = normalizedStrategy
	return true
}

// SetAuthPoolRoutingStrategy stores a strategy for a specific auth pool path.
func (cfg *Config) SetAuthPoolRoutingStrategy(path string, strategy string) bool {
	if cfg == nil {
		return false
	}
	normalizedPath := normalizeAuthPoolPath(path)
	if normalizedPath == "" {
		return false
	}
	normalizedStrategy, ok := normalizeAuthPoolRoutingStrategy(strategy)
	...
	cfg.AuthPool.RoutingStrategyByPath[normalizedPath] = normalizedStrategy
	if normalizeAuthPoolPathKey(cfg.CurrentAuthPoolPath()) == normalizeAuthPoolPathKey(normalizedPath) {
		cfg.Routing.Strategy = normalizedStrategy
	}
	return true
}

// ApplyCurrentAuthPoolRoutingStrategy applies the active auth pool routing strategy when available.
func (cfg *Config) ApplyCurrentAuthPoolRoutingStrategy() bool {
	...
	strategy, ok := cfg.AuthPool.RoutingStrategyByPath[active]
	if !ok {
		return false
	}
	normalizedStrategy, ok := normalizeAuthPoolRoutingStrategy(strategy)
...
func authPoolPathExists(paths []string, target string) bool {
	targetKey := normalizeAuthPoolPathKey(target)
	if targetKey == "" {
		return false
	}
	for _, item := range paths {
		if normalizeAuthPoolPathKey(item) == targetKey {
			return true
		}
	}
	return false
...
func normalizeAuthPoolPath(path string) string {
	return pathutil.NormalizePath(path)
}

func normalizeAuthPoolPathKey(path string) string {
	return pathutil.NormalizeCompareKey(path)
}

func normalizeAuthPoolRoutingStrategy(strategy string) (string, bool) {
	switch strings.ToLower(strings.TrimSpace(strategy)) {
