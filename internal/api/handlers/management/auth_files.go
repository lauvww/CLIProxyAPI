...
func (h *Handler) currentAuthPoolScope() string {
	if h == nil || h.cfg == nil {
		return ""
	}
	return pathutil.NormalizePath(h.cfg.CurrentAuthPoolPath())
}

func authFileNameMatches(left string, right string) bool {
...
func (h *Handler) isPathInCurrentAuthPool(path string) bool {
	if h == nil || h.cfg == nil || !h.cfg.AuthPool.Enabled {
		return true
	}

	currentAuthPool := h.currentAuthPoolScope()
	if currentAuthPool == "" {
		return true
	}

	if strings.TrimSpace(path) == "" {
		return !h.cfg.AuthPool.Enabled
	}

	return pathutil.IsWithinScope(path, currentAuthPool)
}

func (h *Handler) authBelongsToCurrentPool(auth *coreauth.Auth) bool {
	if auth == nil {
		return false
	}
	if h == nil || h.cfg == nil || !h.cfg.AuthPool.Enabled {
...
func (h *Handler) findScopedAuthByName(name string) *coreauth.Auth {
	...
	if auth, ok := h.authManager.GetByID(name); ok && h.authBelongsToCurrentPool(auth) {
		return auth
	}

	auths := h.authManager.List()
	for _, auth := range auths {
		if auth == nil || !h.authBelongsToCurrentPool(auth) {
			continue
		}
		if authFileNameMatches(auth.FileName, name) {
			return auth
		}
		if authFileNameMatches(filepath.Base(strings.TrimSpace(authAttribute(auth, "path"))), name) {
			return auth
		}
	}
...
func (h *Handler) shouldIncludeAuthFileEntry(entry gin.H) bool {
...
func (h *Handler) GetAuthFileModels(c *gin.Context) {
	...
	var authID string
	if targetAuth := h.findScopedAuthByName(name); targetAuth != nil {
		authID = strings.TrimSpace(targetAuth.ID)
	}

	if authID == "" {
		authID = h.authIDForPath(filepath.Join(h.cfg.AuthDir, filepath.Base(name)))
	}
	if authID == "" {
		authID = name
	}
	...
	for _, e := range entries {
		...
		if info, errInfo := e.Info(); errInfo == nil {
			...
			if data, errRead := os.ReadFile(full); errRead == nil {
				...
				for _, candidate := range authIndexCandidates {
					if candidate != "" {
						fileData["auth_index"] = candidate
						break
					}
				}
...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth) gin.H {
	...
	if path != "" {
		...
		entry["full_path"] = resolvedPath
		entry["fullPath"] = resolvedPath
		entry["absolute_path"] = resolvedPath
		entry["absolutePath"] = resolvedPath
		entry["source"] = "file"
...
func (h *Handler) UploadAuthFile(c *gin.Context) {
	...
	if len(fileHeaders) == 1 {
		...
		c.JSON(http.StatusOK, gin.H{"status": "ok", "current_auth_pool": h.currentAuthPoolScope()})
		return
	}
	if len(fileHeaders) > 1 {
		...
		if len(failed) > 0 {
			c.JSON(http.StatusMultiStatus, gin.H{
				...
				"files":             uploaded,
				"failed":            failed,
				"current_auth_pool": h.currentAuthPoolScope(),
			})
			return
		}
		c.JSON(http.StatusOK, gin.H{"status": "ok", "uploaded": len(uploaded), "files": uploaded, "current_auth_pool": h.currentAuthPoolScope()})
		return
	}
	...
	c.JSON(200, gin.H{"status": "ok", "current_auth_pool": h.currentAuthPoolScope()})
}
...
func (h *Handler) DeleteAuthFile(c *gin.Context) {
	...
	if all := c.Query("all"); all == "true" || all == "1" || all == "*" {
		...
		c.JSON(200, gin.H{"status": "ok", "deleted": deleted, "current_auth_pool": h.currentAuthPoolScope()})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "deleted": len(deletedFiles), "files": deletedFiles, "current_auth_pool": h.currentAuthPoolScope()})
}
...
func (h *Handler) writeAuthFile(ctx context.Context, name string, data []byte) error {
	...
	if errMkdir := os.MkdirAll(filepath.Dir(dst), 0o755); errMkdir != nil {
...
func (h *Handler) findAuthForDelete(name string) *coreauth.Auth {
	...
	return h.findScopedAuthByName(name)
}
...
func (h *Handler) PatchAuthFileStatus(c *gin.Context) {
	...
	targetAuth = h.findScopedAuthByName(name)
	...
	c.JSON(http.StatusOK, gin.H{"status": "ok", "disabled": *req.Disabled, "current_auth_pool": h.currentAuthPoolScope()})
}
