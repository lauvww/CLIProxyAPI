...
func NewUsageReporter(ctx context.Context, provider, model string, auth *cliproxyauth.Auth) *UsageReporter {
	...
	if auth != nil {
		...
		reporter.authPool = resolveUsageAuthPool(auth)
		reporter.poolType = resolveUsagePoolType(auth)
		reporter.planType = resolveUsagePlanType(auth)
	}
	return reporter
}
...
func resolveUsagePoolType(auth *cliproxyauth.Auth) string {
	if auth == nil {
		return "custom"
	}
	if auth.Attributes != nil {
		if rawPoolType := strings.TrimSpace(auth.Attributes["pool_type"]); rawPoolType != "" {
			if normalized, ok := normalizeUsagePoolType(rawPoolType); ok {
				return normalized
			}
		}
		if rawPath := strings.TrimSpace(auth.Attributes["path"]); rawPath != "" {
			return inferUsagePoolTypeFromPath(rawPath)
		}
		if rawPath := strings.TrimSpace(auth.Attributes["source"]); rawPath != "" {
			return inferUsagePoolTypeFromPath(rawPath)
		}
	}
	return "custom"
...
func resolveUsageAuthPool(auth *cliproxyauth.Auth) string {
	if auth == nil || auth.Attributes == nil {
		return ""
	}
	if rawAuthPool := strings.TrimSpace(auth.Attributes["auth_pool"]); rawAuthPool != "" {
		return normalizeUsageAuthPool(rawAuthPool)
	}
	if rawPath := strings.TrimSpace(auth.Attributes["path"]); rawPath != "" {
		return inferUsageAuthPoolFromPath(rawPath)
	}
	if rawPath := strings.TrimSpace(auth.Attributes["source"]); rawPath != "" {
		return inferUsageAuthPoolFromPath(rawPath)
	}
...
func inferUsageAuthPoolFromPath(path string) string {
	normalized := normalizeUsageAuthPool(path)
	if normalized == "" {
		return ""
	}
	...
	return normalizeUsageAuthPool(normalized[:lastSlash])
}

func normalizeUsageAuthPool(value string) string {
...
func resolveUsagePlanType(auth *cliproxyauth.Auth) string {
	if auth == nil {
		return ""
	}
	if auth.Attributes != nil {
	...
	if raw, ok := auth.Metadata["plan_type"]; ok {
		if planType, okStr := raw.(string); okStr {
...
func inferUsagePoolTypeFromPath(path string) string {
	...
	for idx := len(parts) - 1; idx >= 0; idx-- {
		part := strings.ToLower(strings.TrimSpace(parts[idx]))
		switch part {
		case "plus":
			return "plus"
		case "free":
...
func normalizeUsagePoolType(value string) (string, bool) {
