...
func TestResolvePersistenceFilePathUsesPoolSpecificName(t *testing.T) {
	...
	if got := filepath.Base(poolAPath); !strings.HasPrefix(got, usagePersistenceFileName+".") || !strings.HasSuffix(got, usagePersistenceFileExt) || strings.Contains(got, "pool-a") {
		t.Fatalf("expected hash-only pool-specific file name, got %q", got)
	}
	if filepath.Base(defaultPath) != usagePersistenceFileName+"."+usagePersistenceDefaultTag+usagePersistenceFileExt {
