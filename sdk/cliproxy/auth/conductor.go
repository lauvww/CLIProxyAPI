...
// Forget removes an auth entry from runtime memory and scheduler state without
// touching the backing store. This is used for auth directory switches where
// credentials remain on disk but must stop participating in the current runtime.
