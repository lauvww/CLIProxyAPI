...
const usagePersistenceWindowDays = 7
...
func (s *Service) Shutdown(ctx context.Context) error {
	...
	s.shutdownOnce.Do(func() {
		...
		if errPersist := s.persistUsageSnapshot(time.Now().UTC()); errPersist != nil {
...
func (s *Service) restorePersistedUsage(now time.Time) {
...
func (s *Service) persistUsageSnapshot(now time.Time) error {
	stats := internalusage.GetRequestStatistics()
	if stats == nil {
		return nil
	}
	...
	poolsToPersist := make([]string, 0, 1)
	...
	appendPool := func(pool string) {
		...
		poolsToPersist = append(poolsToPersist, pool)
	}
	appendPool(currentAuthPool)
	for _, authPool := range internalusage.ListAuthPools(snapshot) {
		appendPool(authPool)
	}
	if len(poolsToPersist) == 0 {
		appendPool("")
	}

	savedPaths := make([]string, 0, len(poolsToPersist))
	var saveErr error
	for _, authPool := range poolsToPersist {
		path, errSave := internalusage.SaveSnapshot(configPath, authPool, snapshot, now)
		if errSave != nil {
			saveErr = errors.Join(saveErr, errSave)
			continue
		}
	...
	log.Infof(
		"saved usage statistics to %d file(s) (current_pool=%q requests=%d)",
		len(savedPaths),
		strings.TrimSpace(currentAuthPool),
...
// PersistUsageSnapshot stores the current in-memory usage snapshot to disk.
func (s *Service) PersistUsageSnapshot(now time.Time) error {
	if s == nil {
		return nil
	}
	return s.persistUsageSnapshot(now)
}
