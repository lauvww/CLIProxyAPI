...
func syncLoggingToFileState(cfg *config.Config, configFilePath string, trayMode bool) {
	if cfg == nil {
		return
	}
	// Keep runtime and config file consistent by trusting the configured value.
	// Do not force logging-to-file during startup.
	if !trayMode {
		return
	}
	log.Infof("tray mode startup uses configured logging-to-file=%t (config: %s)", cfg.LoggingToFile, strings.TrimSpace(configFilePath))
}
...
func main() {
	autoTrayLaunch := shouldAutoTrayLaunch(os.Args[1:])
	if autoTrayLaunch {
		detachConsoleWindow()
	}
	if !autoTrayLaunch {
	...
	if vertexImport != "" {
		...
		if trayMode && tuiMode {
			log.Warn("Tray mode is incompatible with TUI mode; continuing in tray mode")
			tuiMode = false
		}
		if tuiMode {
			if standalone {
